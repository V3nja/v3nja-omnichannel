...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!tokenData.access_token) {
      ...
      return NextResponse.redirect(
        `${protocol}://${host}/channels?error=${encodeURIComponent(
          tokenData.error?.message || "Failed to exchange token"
        )}`
      );
    }
    ...
    if (pages.length === 0) {
      // If no pages returned, create fallback connection record
      ...
      // Save all connected Facebook Pages to DB
      for (const page of pages) {
        ...
        // Ensure default 5 release campaigns exist for this page
        const existingAutos = await prisma.pageAutomation.findMany({
          where: { facebookPageId: savedPage.id },
        });
    ...
    return NextResponse.redirect(
      `${protocol}://${host}/channels?status=success&channel=facebook&count=${pages.length}`
    );
  } catch (err: any) {
    console.error("[FB Callback Processing Error]", err);
    return NextResponse.redirect(
      `${protocol}://${host}/channels?error=${encodeURIComponent(
